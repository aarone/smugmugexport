...
+(NSURLRequest *)smRequestWithURL:(NSURL *)aUrl {
	...
	[req setValue:[SMERequest UserAgent] forHTTPHeaderField:@"User-Agent"];
	return req;
}
@end
...
static void ReadStreamClientCallBack(CFReadStreamRef stream, CFStreamEventType type, void *clientCallBackInfo) {
	switch (type) {
		case kCFStreamEventHasBytesAvailable:
			[(SMERequest *)clientCallBackInfo appendToResponse];
			break;			
		case kCFStreamEventEndEncountered:
			[(SMERequest *)clientCallBackInfo transferComplete];
			break;
		case kCFStreamEventErrorOccurred: {
			CFStreamError err = CFReadStreamGetError(stream);
			[(SMERequest *)clientCallBackInfo errorOccurred:&err];
			break;
		} default:
			break;
	}
...
-(void)startImageUpload:(NSDictionary *)args {
	...
	SMEAlbumRef *albumRef = [args objectForKey: SMUploadKeyAlbumRef];
	NSString *caption = [args objectForKey:SMUploadKeyCaption];
	NSArray *keywords = [args objectForKey:SMUploadKeyKeywords];
	NSObject<SMEUploadRequestObserver> *anObserver = [args objectForKey:SMUploadKeyObserver];
	...
	CFHTTPMessageSetHeaderFieldValue(myRequest, CFSTR("User-Agent"), (CFStringRef)[SMERequest UserAgent]);
