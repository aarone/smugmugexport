...
-(NSString *)groupAtIndex:(unsigned)groupIndex {
	...
	while(YES) { 
		...
		NSString *result = [[[NSString alloc] initWithBytes:dest length:buffSize*sizeof(UChar) encoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingUTF16)] autorelease];
		free(dest);
		return result;
	}
}
...
-(NSString *)performReplacementWithString:(NSString *)aReplacementString replaceAll:(BOOL)replacingAll {
	...
	NSString *result = [[[NSString alloc] initWithBytes:destString
											   ...
											   encoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingUTF16)] autorelease];
	free(destString);
	return result;	
}
