...
-(NSString *)stringToSearch {
	return [NSString stringWithUTF16String:[self textToSearch]];
}
...
-(NSString *)description {
	...
	if([self re] != NULL) {
		...
		return [[[NSString alloc] initWithBytes:p length:len encoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingUTF16)] autorelease];
	}
	...
	while(!isDone) {
		...
		if(status == U_BUFFER_OVERFLOW_ERROR) { // buffer was too small, grow it
			...
			for(i=0; i<numberOfComponents; i++) {
				...
				[results addObject:[NSString stringWithUTF16String:offsetStart]];
			}
			isDone = YES;
		}
	}
