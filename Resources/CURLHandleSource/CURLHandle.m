...
//		mResult = curl_easy_setopt(mCURL, CURLOPT_SSL_VERIFYPEER, FALSE);
//			if(mResult) return nil;

		NSString *caPath = [[[NSBundle bundleForClass:[self class]] resourcePath] stringByAppendingPathComponent:@"cacert.pem"];
// TODO use an 8 bit encoded string here in the default encoding
		mResult = curl_easy_setopt(mCURL, CURLOPT_CAINFO, [caPath UTF8String]);
			if(mResult) return nil;
